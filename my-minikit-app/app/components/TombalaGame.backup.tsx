...
export function TombalaGame({ onBack }: TombalaGameProps) {
  ...
  const handlePlaceBet = (number: number) => {
    ...
    if (filledNumbersSet.has(number)) {
      toast.error('Bu sayı zaten seçilmiş!');
      return;
    }

    if (hasPlayerBet) {
      toast.error('Bu oyunda zaten bahis yaptınız!');
      return;
    }

    if (!gameStats?.[1]) { // isActive
      toast.error('Oyun aktif değil!');
      return;
    }
  ...
  useEffect(() => {
    if (txReceipt) {
      toast.success(`Bahis başarılı! Sayınız: ${selectedNumber}`);
  ...
  useEffect(() => {
    if (bettingError) {
      toast.error('Bahis başarısız oldu');
      setSelectedNumber(null);
    }
  }, [bettingError]);
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4">
        <div className="text-center">
          <h2 className="text-2xl font-bold mb-4">Tombala Oyunu</h2>
          <p className="text-gray-600 mb-6">Oyuna katılmak için cüzdanınızı bağlayın</p>
          <button
            onClick={onBack}
      ...
      <div className="max-w-md mx-auto mb-6">
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm mb-6">
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <span className="text-gray-500">Oyun ID:</span>
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-6">
          <p className="text-sm text-yellow-800">
            <strong>Bahis Miktarı:</strong> {betPrice ? formatEther(betPrice) : '0.001'} ETH
          </p>
          <p className="text-xs text-yellow-600 mt-1">
            Her oyuncu sadece bir sayı seçebilir
          </p>
        </div>
        ...
        {hasPlayerBet && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-6">
            <p className="text-sm text-blue-800">
              <strong>Seçtiğiniz Sayı:</strong> {playerNumbersArray?.[0]?.toString()}
            </p>
          </div>
        )}
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm">
          <h3 className="text-lg font-bold mb-4 text-center">Sayılar (1-25)</h3>
        ...
        {(gameStatsLoading || filledNumbersLoading) && (
          <div className="text-center py-4">
            ...
            <p className="text-sm text-gray-500 mt-2">Oyun durumu yükleniyor...</p>
          </div>
        )}
      </div>
    </div>
